...
(defn print-basis
  ...
  [& aliases]
  (let [{:keys [paths libs classpath-paths]} (basis/create-basis (nos/project-deps-file) (vec aliases))]
    (pprint/pprint
     {:aliases         (vec aliases)
      :paths           paths
...
(defn project-namespaces
  ...
  ([aliases] (project-namespaces (nos/project-deps-file) aliases))
  ([deps-file aliases]
...
(defn- task-namespaces
  ...
  (->> (or namespaces
           (paths-namespaces (:paths (or basis (basis/create-basis (nos/project-deps-file) (vec aliases))))))
       (remove (set exclude))))
...
(defn compile-project
  ...
  (let [basis (when (seq aliases) (nos/establish-deps-edn (nos/project-deps-file) aliases))
