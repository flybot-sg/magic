...
| [magic-unity](./magic-unity) | Unity integration package (IL2CPP support). Ships both Clojure runtimes; a define symbol selects the Editor's — see [Unity integration](./docs/unity-integration.md). | C# |
...
**`magic-unity` UPM package**: the play-time Clojure runtime plus the IL2CPP build pre-processor, loaded by Unity inside a project. Pin `?path=magic-unity#<tag>` as a UPM git URL in `Packages/manifest.json` — one package, for every project.

It ships **both** Clojure runtimes, and a scripting define symbol decides which one the **Editor** loads:

- **unset** (default): the Editor runs stock ClojureCLR, for REPL / hot-reload work.
- **`MAGIC_RUNTIME_IN_EDITOR`**: the Editor runs MAGIC, for Play mode and edit-mode tooling. Set it from `MAGIC > Editor Runtime > Use MAGIC in the Editor`.

Player builds always run MAGIC and are identical either way. Full rationale in [Unity integration](./docs/unity-integration.md) and [docs/dual-runtimes.md](./docs/dual-runtimes.md).
