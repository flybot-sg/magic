...
The obvious staleness is easy to guard: someone edits let's say `clojure/spec/alpha.clj` and forgets to recompile `clojure.spec.alpha.clj.dll`. Hashing the source file catches that, and that is what we used to do, with two SHA256 manifests.
...
With that, compiling the same namespace against the same toolchain produces byte-identical DLLs, verified across the full set of 73 committed reference DLLs. Determinism is what turns `git diff` into a staleness detector.
...
Compared to the manifests this replaced, the mechanism is one moving part instead of three, and it catches the runtime-change case the manifests were structurally blind to. Re-running the hasheq scenario on purpose flags 11 stale DLLs, `clojure.spec.alpha` among them.
