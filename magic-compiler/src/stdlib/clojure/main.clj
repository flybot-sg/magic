...
(defn- java-loc->source
  "Convert a class name and method symbol to a source symbol, either a
  Clojure function or a class and method."
  [clazz method]
  (if (#{'invoke 'invokeStatic} method)
    (let [degen #(.Replace #"--.*$" ^String % "")                                              ;;; #(.replaceAll ^String % "--.*$" "")
      ...
      (symbol ns-name (String/Join "$" ^|System.String[]| (into-array String (cons fn-name nested)))))   ;;; String/join
...
(defn ex-str
  ...
  (let [loc (str (or source "REPL") ":" (or line 1) (if column (str ":" column) ""))
        ...
        simple-class (if class (or (first (re-find #"([^.])+$" class-name)) class-name))         ;;; JVM: #"([^.])++$"
